...

	{
		...
		if ( _nextclass in _classnames_to_save ) then {
			...
			if ( _nextclass in _building_classnames ) then {						
				...
				_EHkilledIdx1 = _nextbuilding addEventHandler ["killed", 	{ diag_log (format ["[INFORMATION] Vehicle: %1 was killed by % 2. instigator = %3", (_this select 0), (_this select 1), (_this select 2) ]); }]; 
				_EHkilledIdx2 = _nextbuilding addEventHandler ["Hit", 		{ diag_log (format ["[INFORMATION] Vehicle: %1 was hit by % 2. instigator = %3", (_this select 0), (_this select 1), (_this select 3) ]);    }]; 
			};
			...
			if ( _nextclass == FOB_typename ) then {
				...
				if (!isMultiplayer) then {
					_markerstr = createMarker [(format ["_respawnfob_%1_%2", FOB_typename, _respawnid]), getPos _nextbuilding];
					_markerstr setMarkerType "flag_Spain";
