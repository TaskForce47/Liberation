...
while { _idx < _nbcivs } do {
	...
	(civilians select (floor (random (count civilians)))) createUnit [_spawnpos, _grp,"", 0.5, "private"];
	_nextciv = ((units _grp) select 0);
	_createdcivs pushBack _nextciv;
	_nextciv addMPEventHandler ["mpkilled", { if (( side (_this select 1)) == TF47_helper_playerFaction ) then { if (isServer) then {combat_readiness = combat_readiness + (2 + (ceil (random 3))); publicVariable "combat_readiness"; };}; }];
