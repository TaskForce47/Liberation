	...
	if ( _overlayshown ) then {
		...
		if ( _uiticks % 25 == 0 ) then {
			...
			if ( _nearest_active_sector != "" ) then {
				...
				if ( [ markerpos _nearest_active_sector, _zone_size ] call F_sectorOwnership == TF47_helper_playerFaction ) then { _colorzone = "ColorBLUFOR" };
				if ( [ markerpos _nearest_active_sector, _zone_size ] call F_sectorOwnership ==TF47_helper_opposingFaction) then { _colorzone = "ColorOPFOR" };
