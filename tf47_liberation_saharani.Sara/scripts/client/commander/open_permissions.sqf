	...
	if ( permission_playerid != -1 || permission_toset != -1 ) then {
		...
		if ( _player_uid != "" ) then {
			...
			if ( permission_toset == 666 ) then {
				_player_permissions = [ true, true, true, true, true, true, false ];
			};
			if ( permission_toset == 999 ) then {
				_player_permissions = [ false, false, false, false, false, false, false ];
			};

			if ( _player_idx == -1 ) then {

				if ( permission_toset != 666 && permission_toset != 999 ) then {
					_player_permissions  = ([_player_uid]call TF47_GetPlayer)  getVariable ["TF47_Permissions", [true, false, false, false, true, true, false]];
					_player_permissions set [ permission_toset, true ];
				};
				([_player_uid]call TF47_GetPlayer) setVariable ["TF47_Permissions", _player_permissions, true];
