...
while {alive _Unit && VCOM_UseMarkers} do 
{
	...
	if ((side _Unit) isEqualTo TF47_helper_playerFaction ) then {_marker1 setmarkercolor "ColorRed";};
	if ((side _Unit) isEqualTo TF47_helper_opposingFaction) then {_marker1 setmarkercolor "ColorBlue";};
...
_marker1 = createMarker [_marker1Names,(getPosASL _Unit)];
_marker1 setmarkershape "ELLIPSE";
if ((side _Unit) isEqualTo TF47_helper_playerFaction ) then {_marker1 setmarkercolor "ColorOrange";};
if ((side _Unit) isEqualTo TF47_helper_opposingFaction) then {_marker1 setmarkercolor "ColorOrange";};
