...
[
	{
		_myfpsmarker = (_this select 0) select 0;
		_sourcestr = (_this select 0) select 1;
		_myfps = diag_fps;
		_localunits = { local _x } count allUnits;
		_localvehicles = { local _x } count (vehicles select { (_x isKindOf "LandVehicle") || (_x isKindOf "Air") });
