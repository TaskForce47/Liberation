...
if(_clearedtobuildfob) then {
	while { (_idx < (count sectors_allSectors)) && _clearedtobuildsector } do {
		if ( player distance2D (getmarkerpos (sectors_allSectors select _idx)) < _minsectordist ) then {
			if ( "military" in ((sectors_allSectors select _idx) splitString "_")) then {
				_clearedtobuildsector = true;
			} else {
				_clearedtobuildsector = false;
			};
			_distsector = player distance2D (getmarkerpos (sectors_allSectors select _idx));
		};
