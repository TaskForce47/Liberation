...
if ( _status == 1 ) then {
	if ((markerText _sector) == "Radiotower") then {
		_str = format ["Radiotower %1", mapGridPosition (getMarkerPos _sector)];
		[ "lib_sector_attacked", [ _str ] ] call BIS_fnc_showNotification;
	} else {
